...
module.exports = {
    async createDeviceInfo(box, element, deviceInfo) {
        try {
            let common = {};
            let icons;
            if (element && element.picture && element.picture != "") {
                icons = element.picture;
            } else if (deviceInfo && deviceInfo.boxtype && cs.icon[deviceInfo.boxtype]) {
                icons = cs.icon[deviceInfo.boxtype];
            } else {
                icons = cs.icon["unknown"];
            }
            common = {
                name: deviceInfo.boxtype,
                desc: box,
                icon: icons,
            };
            await this.createDataPoint(box, common, "device");
            common = {
                type: "number",
                role: "info.status",
                name: {
            ...
            await this.createDataPoint(`${box}.deviceInfo`, common, "folder");
            ...
            this.log_translator("error", "try", `createDeviceInfo: ${e}`);
        }
    },
    async createStatusInfo(box, statusInfo, deviceInfo, element) {
        try {
            let icons;
            let common = {};
            if (element && element.picture && element.picture != "") {
            ...
            await this.createDataPoint(`${box}.statusInfo`, common, "folder");
            ...
            this.log_translator("error", "try", `createStatusInfo: ${e}`);
        }
    },
    async createRemote(box, deviceInfo) {
        try {
            ...
            if (deviceInfo.webifver && deviceInfo.webifver.match(/wif/gi) != null) {
                common = {
                    name: {
                        en: "Snapshot",
                        de: "Schnappschuss",
                ...
                if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                    if (this.alexa[box]["SNAPSHOT"] && this.alexa[box]["SNAPSHOT"] != "") {
                        common["smartName"] = {};
                        common["smartName"][this.lang] = this.alexa[box]["SNAPSHOT"];
                ...
                if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                    if (this.alexa[box]["SNAPSHOT_OSD"] && this.alexa[box]["SNAPSHOT_OSD"] != "") {
                        common["smartName"] = {};
                        common["smartName"][this.lang] = this.alexa[box]["SNAPSHOT_OSD"];
                        common["smartName"]["smartType"] = "SWITCH";
                    }
                }
            ...
            if (this.config.mac) {
                ...
                if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                    if (this.alexa[box]["WOL"] && this.alexa[box]["WOL"] != "") {
                        common["smartName"] = {};
                        common["smartName"][this.lang] = this.alexa[box]["WOL"];
                        common["smartName"]["smartType"] = "SWITCH";
                    }
                }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["STANDBY"] && this.alexa[box]["STANDBY"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["STANDBY"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["WAKEUP"] && this.alexa[box]["WAKEUP"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["WAKEUP"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["REBOOT_ENIGMA"] && this.alexa[box]["REBOOT_ENIGMA"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["REBOOT_ENIGMA"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["REBOOT"] && this.alexa[box]["REBOOT"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["REBOOT"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["DEEP_STANDBY"] && this.alexa[box]["DEEP_STANDBY"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["DEEP_STANDBY"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                if (this.alexa[box]["TOGGLE_STANDBY"] && this.alexa[box]["TOGGLE_STANDBY"] != "") {
                    common["smartName"] = {};
                    common["smartName"][this.lang] = this.alexa[box]["TOGGLE_STANDBY"];
                    common["smartName"]["smartType"] = "SWITCH";
                }
            }
            ...
            const commons = [
                ...
                },
                {
                    ...
                    name: {
                        ...
                        ru: "Поверните на устройство",
                        pt: "Ligue o dispositivo",
                        nl: "Zet het apparaat aan",
                        fr: "Allume le dispositif",
                        it: "Accendere il dispositivo",
                        es: "Activar el dispositivo",
                        pl: "Odwraca się od urządzenia",
                        uk: "Увімкніть пристрій",
                        "zh-cn": "关于装置的说明",
                    },
                },
                {
                    datapoint: "KEY_POWER2",
                    role: "button.power",
                    name: {
                        en: "Turn off device",
                        ...
                        nl: "Zet uit",
                        fr: "Arrêt du dispositif",
                        it: "Spegnere il dispositivo",
                        es: "Apaga el dispositivo",
                        pl: "Odwracać się od urządzenia",
            ...
            for (const element of commons) {
                common = {
                    ...
                    role: element.role,
                    type: "boolean",
                    desc: `${element.name.en} Button`,
                    read: true,
                    write: true,
                    def: false,
                };
                if (this.alexa && this.alexa[box] && this.alexa[box].alexa) {
                    if (this.alexa[box][element.datapoint] && this.alexa[box][element.datapoint] != "") {
                        common["smartName"] = {};
                        common["smartName"][this.lang] = this.alexa[box][element.datapoint];
                        common["smartName"]["smartType"] = "SWITCH";
                    } else if (
                        this.alexa[box][element.datapoint] &&
                        this.alexa[box][element.datapoint] == "" &&
            ...
            common = {
                ...
                name: {
                    en: "Interval for SSH",
                    de: "Intervall für SSH",
                    ru: "Интервал для SSH",
                    pt: "Intervalo para SSH",
                    nl: "Interval voor SSH",
                    fr: "Intervalle pour SSH",
                    it: "Intervallo per SSH",
                    es: "Intervalo para SSH",
                    pl: "Interwaluacja SSH",
                    uk: "Інтервал для SSH",
                    "zh-cn": "SSH的间隔",
                },
                desc: "Interval for SSH",
                def: 3600,
                min: 1,
                max: 10000,
                read: true,
            ...
            await this.createDataPoint(`${box}.SSH_INTERVAL`, common, "state");
            common = {
                type: "number",
                role: "level.Interval",
                name: {
                    en: "Interval for recordings.",
                    de: "Intervall für Aufnahmen.",
                    ru: "Интервал для записей.",
                    pt: "Intervalo para gravações.",
                    nl: "Interval voor opnames.",
                    fr: "Intervalle pour enregistrements.",
                    it: "Intervallo per le registrazioni.",
                    es: "Intervalo para grabaciones.",
                    pl: "Interval for recordings (ang.).",
                    uk: "Інтервал для записів.",
                    "zh-cn": "录制。.",
                },
                desc: "Interval for recordings.",
                def: 60,
                min: 1,
                max: 1000,
                read: true,
            ...
            await this.createDataPoint(`${box}.RECORD_INTERVAL`, common, "state");
            common = {
                type: "number",
                role: "level.Interval",
                name: {
                    en: "Interval for standby.",
                    de: "Intervall für Standby.",
                    ru: "Интервал для ожидания.",
                    pt: "Intervalo de espera.",
                    nl: "Interval voor stand-by.",
                    fr: "Intervalle pour standby.",
                    it: "Intervallo per standby.",
                    es: "Intervalo para espera.",
                    pl: "Interval for standby (ang.).",
                    uk: "Інтервал для очікування.",
                    "zh-cn": "待命汇率。.",
                },
                desc: "Interval for standby",
                def: 60,
                min: 1,
                max: 1000,
                read: true,
            ...
            await this.createDataPoint(`${box}.STANDBY_INTERVAL`, common, "state");
            common = {
                type: "number",
                role: "level.Interval",
                name: {
                    en: "Interval for online.",
                    de: "Intervall für online.",
                    ru: "Интервал для онлайн.",
                    pt: "Intervalo online.",
                    nl: "Interval voor online.",
                    fr: "Intervalle pour en ligne.",
                    it: "Intervallo online.",
                    es: "Intervalo para internet.",
                    pl: "Interval for online (ang.).",
                    uk: "Інтервал онлайн.",
                    "zh-cn": "网上汇率。.",
                },
                desc: "Interval for Online",
                def: 5,
                min: 0,
                max: 240,
                read: true,
            ...
            await this.createDataPoint(`${box}.ONLINE_INTERVAL`, common, "state");
            common = {
                type: "string",
                role: "info",
                name: {
            ...
            this.log_translator("error", "try", `createRemote: ${e}`);
        }
    },
    async createBouquetsAndEPG(box, bouquets) {
        try {
            ...
            this.log_translator("error", "try", `createBouquetsAndEPG: ${e}`);
        }
    },
    async createFolderJson(folder, deviceId) {
        try {
            this.log_translator("info", "Folder", folder);
            const movielist = await this.getRequest(`${cs.API.movielist}?dirname=${encodeURI(folder)}`, deviceId);
            if (movielist && movielist.movies && Array.isArray(movielist.movies) && movielist.movies.length > 0) {
            ...
            if (
                ...
                for (const element of movielist.bookmarks) {
                    this.log_translator("debug", "Element", element);
            ...
            this.log_translator("error", "try", `createFolderJson: ${e}`);
        }
    },
    ...
    async loadBouquets(box, bouquets) {
        try {
            ...
            this.log_translator("error", "try", `loadBouquets: ${e}`);
        }
    },
    async statesLoadTimer(box, states) {
        try {
            ...
            this.log_translator("error", "try", `statesLoadTimer: ${e}`);
        }
    },
    async statesSetFolder(box, states) {
        try {
            ...
            this.log_translator("error", "try", `statesSetFolder: ${e}`);
        }
    },
    async createTimerFolder(box) {
        try {
            ...
            this.log_translator("error", "try", `createTimerFolder: ${e}`);
        }
    },
    async createAlexaFolder(box) {
        try {
            const common = {
                name: {
                    en: "Own voice commands",
                    de: "Eigene Sprachbefehle",
                    ru: "Собственные голосовые команды",
                    pt: "Comandos de voz próprios",
                    nl: "Eigen stemcommandant",
                    fr: "Des commandes vocales",
                    it: "Comandi vocali propri",
                    es: "Los comandos de voz propios",
                    pl: "Komenda głosowa",
                    uk: "Власні голосові команди",
                    "zh-cn": "无声音指挥",
                },
            };
            await this.createDataPoint(`${box}.remote.alexa`, common, "folder");
        } catch (e) {
            this.log_translator("error", "try", `createAlexaFolder: ${e}`);
        }
    },
};
