...
module.exports = class Json2iob {
    ...
    async parse(path, element, options, checkelement, check) {
        try {
            ...
            if (Array.isArray(element)) {
                await this.extractArray(element, "", path, options, checkelement, check);
                return;
            }
            ...
            for (const key of objectKeys) {
                if (this.isJsonString(element[key]) && options.autoCast) {
                    ...
                    checkelement[key] = JSONbig.parse(checkelement[key]);
                }

                if (Array.isArray(element[key])) {
                    await this.extractArray(element, key, path, options, checkelement, check);
                } else if (element[key] !== null && typeof element[key] === "object") {
                    await this.parse(path + "." + key, element[key], options, checkelement[key], check);
                } else {
                    ...
                    if (key === "longdesc") {
                        ...
                        checkelement[key] = checkelement[key].toString().replace(/&quot;/g, '"');
                    }
                    if (checkelement[key] != element[key] || !check) {
                        if (
                            ...
                            try {
                                ...
                                this.checkTypes[path + "." + key] = type;
                                this.adapter.log.info(obj.common.type + " Change " + type + " - " + path + "." + key);
                            } catch (error) {
                                this.adapter.log.error("Error change types: " + path + " " + JSON.stringify(element));
                                this.adapter.log.error("Error change checktypes: " + path + " " + JSON.stringify(checkelement));
                                this.adapter.log.error(error);
                            }
                        }
            ...
            this.adapter.log.error("Error extract checkkeys: " + path + " " + JSON.stringify(checkelement));
            this.adapter.log.error(error);
        }
    }
    async extractArray(element, key, path, options, checkelement, check) {
        try {
            if (key) {
                element = element[key];
                checkelement = checkelement[key];
            }
            for (let index in element) {
                const arrayElement = element[index];
                const checkarrayElement = checkelement[index];
                ...
                if (typeof arrayElement === "string" && key !== "") {
                    await this.parse(path + "." + key + "." + arrayElement, arrayElement, options, checkarrayElement, check);
                    continue;
                }
                ...
                await this.parse(path + "." + arrayPath, arrayElement, options, checkarrayElement, check);
            }
        } catch (error) {
